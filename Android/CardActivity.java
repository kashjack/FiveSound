...
public class CardActivity extends BaseHeadActivity {
    ...
    public void onCreate(@Nullable Bundle bundle) {
        ...
        setContentView((int) R.layout.activity_card);
        initView();
        init();
    }
    ...
    public void initView() {
        super.initView();
        setUiTitle(R.string.activity_card_title);
        ...
        this.seek_song.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onStartTrackingTouch(SeekBar seekBar) {
                boolean unused = CardActivity.this.mShouldUpdateProgress = false;
            }

            public void onStopTrackingTouch(SeekBar seekBar) {
                int progress = seekBar.getProgress();
                int access$100 = (CardActivity.this.totalTime * progress) / 100;
                Log.i("ACV_RADIO", "seek_song set play time:" + access$100 + ",progress:" + progress + ",total time:" + CardActivity.this.totalTime);
                CardActivity.this.sendCommand(Command.setPlayTime(access$100), true);
                boolean unused = CardActivity.this.mShouldUpdateProgress = true;
            }
        });
        ...
        this.ckb_play.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton compoundButton, boolean z) {
                if (CardActivity.this.toggleByMan) {
                    CardActivity.this.ckb_play.setChecked(!z);
                    CardActivity.this.sendData(Command.playPause(Command.KEY_PRESS));
                }
            }
        });
        ...
        this.btn_prev.setOnTouchListener(new View.OnTouchListener() {
            public boolean onTouch(View view, MotionEvent motionEvent) {
                if (motionEvent.getAction() == 0) {
                    long unused = CardActivity.this.mBtnPrevDownTime = System.currentTimeMillis();
                    CardActivity.this.btn_prev.setBackground(CardActivity.this.getDrawable(R.drawable.btn_prev_pressed));
                } else if (motionEvent.getAction() == 2) {
                    long currentTimeMillis = System.currentTimeMillis();
                    if (CardActivity.this.mBtnPrevHold || currentTimeMillis - CardActivity.this.mBtnPrevDownTime > 500) {
                        boolean unused2 = CardActivity.this.mBtnPrevHold = true;
                        if (currentTimeMillis - CardActivity.this.mPrevLastMoveTime > 300) {
                            Log.i("ACV_RADIO", "btn_prev hold..");
                            CardActivity.this.sendCommand(Command.prev(Command.KEY_HOLD), true);
                            long unused3 = CardActivity.this.mPrevLastMoveTime = currentTimeMillis;
                        }
                    }
                } else if (motionEvent.getAction() == 1) {
                    if (CardActivity.this.mBtnPrevHold) {
                        Log.i("ACV_RADIO", "btn_prev release..");
                        boolean unused4 = CardActivity.this.mBtnPrevHold = false;
                        CardActivity.this.sendCommand(Command.prev(Command.KEY_RELEASE), true);
                    } else {
                        Log.i("ACV_RADIO", "btn_prev click..");
                        CardActivity.this.sendData(Command.prev(Command.KEY_PRESS));
                    }
                    CardActivity.this.btn_prev.setBackground(CardActivity.this.getDrawable(R.drawable.btn_prev_normal));
                    long unused5 = CardActivity.this.mBtnPrevDownTime = 0;
                    long unused6 = CardActivity.this.mPrevLastMoveTime = 0;
                }
                return true;
            }
        });
        ...
        this.btn_next.setOnTouchListener(new View.OnTouchListener() {
            public boolean onTouch(View view, MotionEvent motionEvent) {
                if (motionEvent.getAction() == 0) {
                    long unused = CardActivity.this.mBtnNextDownTime = System.currentTimeMillis();
                    CardActivity.this.btn_next.setBackground(CardActivity.this.getDrawable(R.drawable.btn_next_pressed));
                } else if (motionEvent.getAction() == 2) {
                    long currentTimeMillis = System.currentTimeMillis();
                    if (CardActivity.this.mBtnNextHold || currentTimeMillis - CardActivity.this.mBtnNextDownTime > 500) {
                        boolean unused2 = CardActivity.this.mBtnNextHold = true;
                        if (currentTimeMillis - CardActivity.this.mNextLastMoveTime > 300) {
                            Log.i("ACV_RADIO", "btn_next hold..");
                            CardActivity.this.sendCommand(Command.next(Command.KEY_HOLD), true);
                            long unused3 = CardActivity.this.mNextLastMoveTime = currentTimeMillis;
                        }
                    }
                } else if (motionEvent.getAction() == 1) {
                    if (CardActivity.this.mBtnNextHold) {
                        Log.i("ACV_RADIO", "btn_next release..");
                        boolean unused4 = CardActivity.this.mBtnNextHold = false;
                        CardActivity.this.sendCommand(Command.next(Command.KEY_RELEASE), true);
                    } else {
                        Log.i("ACV_RADIO", "btn_next click..");
                        CardActivity.this.sendData(Command.next(Command.KEY_PRESS));
                    }
                    CardActivity.this.btn_next.setBackground(CardActivity.this.getDrawable(R.drawable.btn_next_normal));
                    long unused5 = CardActivity.this.mBtnNextDownTime = 0;
                    long unused6 = CardActivity.this.mNextLastMoveTime = 0;
                }
                return true;
            }
        });
    ...
    public void receiveData(BleNotifyData bleNotifyData) {
        ...
        if (bleNotifyData != null) {
            Log.i("ACV_RADIO", "CardActivity-->receiveData cmd:" + CommandParser.parseCmdStr(bleNotifyData.cmdH, bleNotifyData.cmdL) + ",data length:" + bleNotifyData.length);
    ...
    /* renamed from: com.acv.radio.activity.CardActivity$5  reason: invalid class name */
    ...
    private void updatePlayPause(BleNotifyData bleNotifyData) {
        if (bleNotifyData.data == null || bleNotifyData.length != 1) {
            Log.e("ACV_RADIO", "CardActivity GET_PLAY_PAUSE data error..");
            return;
        }
        byte[] bArr = bleNotifyData.data;
        boolean z = false;
        Command.logoutAllByte("CardActivity GET_PLAY_PAUSE data: ", bArr, false);
